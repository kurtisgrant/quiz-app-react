...
const getQuiz = function(db, quizIdentifier) {
  return db
    .query(`SELECT quizzes.id as quiz_id, quizzes.title, users.name as owner, quizzes.description, questions.question, question_options.*
            FROM quizzes
...
const getQuizAttempt = function(db, quizAttemptId) {
  return db
    .query(`SELECT quizzes.*, quiz_attempts.when_completed as attempt_timestamp, questions.*, questions.id as question_id, question_options.*, question_options.id as option_id, question_responses.*, tester_id
            FROM quiz_attempts
            JOIN quizzes ON quizzes.id = quiz_attempts.quiz_id
            JOIN questions ON quizzes.id = questions.quiz_id
            JOIN question_options ON questions.id = question_id
      ...
      for (let option of data) {
        if (option.question_id !== currentQuestion.question_id) {
          currentQuestion = {
            ...
            options: [{ option_id: option.option_id, option_text: option.answer }],
          ...
          currentQuestion.options.push({ option_id: option.option_id, option_text: option.answer });
        }
      }
      ...
      for (option of data) {
        for (let question of attempt.questions) {
          if (question.question_id === option.question_id && option.is_correct === true) {
            ...
            question["correct_option_text"] = option.answer;
          }
        }
      }
...
const insertQuizAttempt = (db, tester_id, quiz_id) => {
  const quizQuery = 'INSERT INTO quiz_attempts(tester_id, quiz_id) VALUES($1, $2) RETURNING id; ';
...
const submitQuiz = async function(db, quizData) {
  ...
  for (const selection of qz.selections) {
    await insertQuestionResponse(db, attempt_id, selection[0], selection[1]);
  }
};
